...
export function AdminCategories() {
  ...
  const columns: Column<Category>[] = [
    { header: 'Nom', accessor: (c) => <span className="font-bold text-[#063F3A]">{c.name}</span> },
    ...
    },
    {
      ...
      accessor: (c) => (
        <div className="flex justify-end gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); handleEdit(c); }} className="text-[#00843D] hover:text-[#c96a1a] hover:bg-orange-50 p-2">
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader 
        ...
        action={
          <Button onClick={() => { setEditingId(null); setName(''); setSlug(''); setDescription(''); setIsFormOpen(true); }} className="bg-[#C8102E] text-white">
      ...
      {isFormOpen && (
        <form onSubmit={handleSave} className="bg-white p-6 rounded-xl border border-stone-200 shadow-sm mb-6 space-y-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-bold text-[#063F3A]">{editingId ? 'Éditer la catégorie' : 'Nouvelle catégorie'}</h3>
          ...
          <div className="flex justify-end pt-4"><Button type="submit" className="bg-[#C8102E] text-white">Enregistrer</Button></div>
        </form>
      )}
